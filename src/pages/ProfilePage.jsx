  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      matchesSnap.forEach((d) => {
        ...
        st.played++; st.gf += myScore; st.ga += oppScore
        let result = 'تعادل'
      ...
      setStats(st); setHistory(hist); setLoading(false)
    })
    return () => unsub()
  }, [navigate])
      ...
      <div style={s.card}>
        ...
        <div style={s.section}>
          ...
          <form onSubmit={handleSaveFb} style={{ display: 'flex', gap: 8 }}>
            <input type="text" placeholder="https://facebook.com/..." value={fbUrl} onChange={(e) => setFbUrl(e.target.value)} style={{ ...s.input, flex: 1 }} />
...
function StatBox({ label, value }) { return <div style={s.statBox}><p style={s.statValue}>{value}</p><p style={s.statLabel}>{label}</p></div> }
