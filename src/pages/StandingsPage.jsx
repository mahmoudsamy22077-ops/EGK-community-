...
const tournamentLabels = { league: 'EGK LEAGUE', championship: 'EGK championship', cup: 'EGK CUP' }

export default function StandingsPage() {
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()
  const tournamentId = searchParams.get('tournament') || 'league'
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      matchesSnap.forEach((d) => {
        const m = d.data()
        const mTournament = m.tournamentId || 'league'
        if (mTournament !== tournamentId) return
      ...
      const list = Object.values(stats).filter((r) => r.played > 0)
      ...
      <div style={s.card}>
        <div style={s.header}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <EgkLogo size={26} />
            <span style={s.title}>ترتيب {tournamentLabels[tournamentId] || tournamentId}</span>
          </div>
          <button onClick={() => navigate('/home')} style={s.backBtn}>رجوع</button>
        </div>
        <div style={s.tabsWrap}>
          {Object.entries(tournamentLabels).map(([id, label]) => (
            <button key={id} onClick={() => navigate('/standings?tournament=' + id)} style={id === tournamentId ? s.tabActive : s.tabInactive}>{label}</button>
          ))}
        </div>
        <div style={{ padding: '1rem 1.1rem' }}>
          {rows.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--egk-text-dim)' }}>لسه مفيش نتائج متسجلة في البطولة دي.</p>
          ) : (
            <table style={s.table}>
              <thead>
                <tr>
                  <th style={s.thNum}>#</th><th style={s.th}>اللاعب</th><th style={s.thCenter}>ل</th><th style={s.thCenter}>ف</th><th style={s.thCenter}>ت</th><th style={s.thCenter}>خ</th><th style={s.thRight}>نقاط</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} style={s.tr}>
                    <td style={s.tdDim}>{i + 1}</td><td style={s.td}>{r.name}</td><td style={s.tdCenter}>{r.played}</td><td style={s.tdCenter}>{r.won}</td><td style={s.tdCenter}>{r.draw}</td><td style={s.tdCenter}>{r.lost}</td><td style={s.tdPoints}>{r.points}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const s = {
  ...
  title: { fontWeight: 500, fontSize: 13, color: 'var(--egk-text-primary)' },
  ...
  tabsWrap: { display: 'flex', borderBottom: '0.5px solid var(--egk-border)' },
  tabActive: { flex: 1, textAlign: 'center', padding: '10px 0', fontSize: 12, fontWeight: 500, color: 'var(--egk-accent)', borderBottom: '2px solid var(--egk-accent)', background: 'transparent', border: 'none' },
  tabInactive: { flex: 1, textAlign: 'center', padding: '10px 0', fontSize: 12, color: 'var(--egk-text-muted)', background: 'transparent', border: 'none' },
