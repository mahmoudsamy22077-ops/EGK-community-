  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.welcome}>
          <p style={{ fontSize: 15, color: 'var(--egk-text-primary)', margin: 0 }}>أهلاً {player?.name || player?.email || ''} 👋</p>
        </div>
        <div style={{ padding: '0 1.1rem 1rem', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/profile')} style={s.standingsBtn}>بروفايلي</button>
          <button onClick={() => navigate('/leaderboards')} style={s.standingsBtn}>Leaderboards</button>
          {canSeeAdmin && <button onClick={() => navigate('/admin')} style={s.adminBtn}>لوحة التحكم</button>}
        </div>
        <div style={s.tabsWrap}>
          <button onClick={() => navigate('/standings?tournament=league')} style={s.tabBtn}>الدوري</button>
          <button onClick={() => navigate('/standings?tournament=championship')} style={s.tabBtn}>دوري الأبطال</button>
          <button onClick={() => navigate('/standings?tournament=cup')} style={s.tabBtn}>الكأس</button>
        </div>
        <div style={{ padding: '1rem 1.1rem' }}>
          <p style={s.sectionTitle}>آخر الأخبار</p>
          <div style={s.newsCard}>
            <p style={{ fontSize: 13, color: 'var(--egk-text-primary)', margin: 0 }}>أهلاً بيك في منصة EGK — البداية كانت هنا 🎉</p>
          </div>
        </div>
      </div>
    </div>
...
const s = {
  ...
  standingsBtn: { flex: 1, padding: '10px 0', background: 'transparent', border: '1px solid var(--egk-accent)', color: 'var(--egk-accent)', borderRadius: 'var(--egk-radius-sm)', fontWeight: 500, fontSize: 12, minWidth: 90 },
  adminBtn: { flex: 1, padding: '10px 0', background: 'var(--egk-accent)', color: 'var(--egk-accent-on)', border: 'none', borderRadius: 'var(--egk-radius-sm)', fontWeight: 500, fontSize: 12, minWidth: 90 },
  tabsWrap: { display: 'flex', borderBottom: '0.5px solid var(--egk-border)', marginTop: 4 },
  tabBtn: { flex: 1, textAlign: 'center', padding: '10px 0', fontSize: 13, color: 'var(--egk-text-muted)', background: 'transparent', border: 'none' },
