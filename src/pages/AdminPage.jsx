...
const tournaments = [
  { id: 'league', label: 'EGK LEAGUE' },
  { id: 'championship', label: 'EGK championship' },
  { id: 'cup', label: 'EGK CUP' },
]
...
export default function AdminPage() {
  ...
  const [tournamentId, setTournamentId] = useState('league')
  ...
  async function handleSaveMatch(e) {
    e.preventDefault()
    setSaveMsg('')
    if (!homeId || !awayId || homeScore === '' || awayScore === '') { setSaveMsg('اختار اللاعبين واكتب النتيجة كاملة'); return }
    if (homeId === awayId) { setSaveMsg('لازم تختار لاعبين مختلفين'); return }
    try {
    ...
    } catch (err) { setSaveMsg('خطأ: ' + err.message) }
  }
  ...
  async function handleRoleChange(playerId, newRole) {
    ...
    } catch (err) { setRoleMsg('خطأ: ' + err.message) }
  }

  async function handleAddPenalty(e) {
    e.preventDefault()
    setPenMsg('')
    if (!penPlayerId || penPoints === '' || !penReason.trim()) { setPenMsg('اختار اللاعب واكتب النقط والسبب'); return }
    try {
      await addDoc(collection(db, 'penalties'), {
        playerId: penPlayerId, points: Number(penPoints), reason: penReason.trim(), appliedAt: serverTimestamp(),
      })
      setPenMsg('اتسجلت العقوبة ✅')
      setPenPlayerId(''); setPenPoints(''); setPenReason('')
      await loadPenalties(players)
    } catch (err) { setPenMsg('خطأ: ' + err.message) }
  }
      ...
      <div style={s.card}>
        ...
        <div style={s.section}>
          ...
          <form onSubmit={handleSaveMatch}>
            <label style={s.label}>البطولة</label>
            <select value={tournamentId} onChange={(e) => setTournamentId(e.target.value)} style={s.select}>
